...
#ruby_block "Start writing to file" do 
# block do
#      $stdout.reopen(File.new('/var/log/chef_run.log', 'w'))
# end
#end
...
containerId = node["rlcatalyst"]["containerId"] || RandomWord.nouns.next
...
ruby_block "Update App data" do
        block do
                # $stdout = STDOUT
                # output = File.readlines("/var/log/chef_run.log")
                # node.default['app_data_handler']['app']['appLogs'] = output
